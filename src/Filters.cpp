...
FilterCoefficients butterworth(size_t order, float cutoff, float sample_rate, bool hpf)
{
   ...
   // Use return_value_policy::copy to ensure data is copied
    std::vector<float> b = result[0].cast<std::vector<float>>(py::return_value_policy::copy);
    std::vector<float> a = result[1].cast<std::vector<float>>(py::return_value_policy::copy);
